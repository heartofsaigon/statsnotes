...
matplot(x_pr, t(y_pr), type = "l", lty = 1, xlab = "prior x", ylab = "density", ylim = c(-6,6))
...
*Why do we need to consider the hyperparamters in GP?* 
...
#| label: fig-04
#| eval: true
#| fig-cap: "The predictive surface interpolates the data as the scale of 10 is added."

y_obs_sc = y_obs*8

mu_pos = mu_pr + kap%*%solve(Sigma_obs)%*%(y_obs_sc)
Sigma_pos = Sigma_pr - kap%*%solve(Sigma_obs)%*%t(kap)
...
matplot(x_pr, t(ci), col = "black", lty = 1, add = T, type = "l")
#lines(x, m )
lines(x_obs, y_obs_sc, pch = 16, col = "red", type = "o", cex = 1.2)
```
